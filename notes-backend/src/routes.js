...
noteRouter.post('/', async (req, res) => {
    try {
        ...
        if (!notebookId) {
            ...
            try {
                ...
                if (jsonError.status === 404) {
                    ...
                    console.error({
                        message: 
                            'Error veryfying the notebook ID. Upstream notebooks service not available. Storing note with provided ID for later validation.',
                        notebookId,
                        error: err.message,
                    });
                }
        ...
        if (!title || !content) {
            return res
                .status(400)
                .json({ error: "'title', 'content' field are required"});
        }
